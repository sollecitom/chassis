...
package org.sollecitom.chassis.logger.slf4j.adapter
...
class Slf4jLoggerAdapter(private val logger: Logger) : Slf4jLogger {
    ...
    override fun isTraceEnabled() = with(logger) { LoggingLevel.TRACE.isEnabledForLoggerName(logger.name) }
    ...
    override fun isDebugEnabled() = with(logger) { LoggingLevel.DEBUG.isEnabledForLoggerName(logger.name) }
    ...
    override fun isInfoEnabled() = with(logger) { LoggingLevel.INFO.isEnabledForLoggerName(logger.name) }
    ...
    override fun isWarnEnabled() = with(logger) { LoggingLevel.WARN.isEnabledForLoggerName(logger.name) }
    ...
    override fun isErrorEnabled() = with(logger) { LoggingLevel.ERROR.isEnabledForLoggerName(logger.name) }
