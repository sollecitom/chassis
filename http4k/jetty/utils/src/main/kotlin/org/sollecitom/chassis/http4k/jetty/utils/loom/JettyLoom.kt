...
class JettyLoom(private val port: Int, override val stopMode: ServerConfig.StopMode, private val server: Server) : PolyServerConfig {
    ...
    constructor(port: Int, stopMode: ServerConfig.StopMode, vararg inConnectors: ConnectorBuilder) : this(port, stopMode, Server(LoomThreadPool()).apply {
        inConnectors.forEach { addConnector(it(this)) }
    })

    constructor(port: Int, threadPool: ThreadPool) : this(port, ServerConfig.StopMode.Graceful(Duration.ofSeconds(5)), Server(threadPool).apply { addConnector(http(port)(this)) })
    ...
    override fun toServer(http: HttpHandler?, ws: WsHandler?, sse: SseHandler?): Http4kServer {
        ...
        ws?.let {
            server.insertHandler(WebSocketUpgradeHandler(WebSocketComponents()).apply {
...
fun HttpHandler.toJettyHandler(withStatisticsHandler: Boolean = false): HandlerWrapper = ServletContextHandler(ServletContextHandler.SESSIONS).apply {
...
private fun http(httpPort: Int): ConnectorBuilder = { server: Server -> ServerConnector(server).apply { port = httpPort } }