...
import org.sollecitom.chassis.cryptography.implementation.bouncycastle.asymmetric.signing.JavaSigningPrivateKeyAdapter
...
internal class DilithiumKeyPairFactory(private val random: SecureRandom) : KeyPairFactory<Dilithium.KeyPairArguments, SigningPrivateKey, VerifyingPublicKey> {
    ...
    override fun fromKeys(privateKey: SigningPrivateKey, publicKey: VerifyingPublicKey): AsymmetricKeyPair<SigningPrivateKey, VerifyingPublicKey> {
        ...
        require(privateKey.metadata.algorithm == Dilithium.NAME) { "Private key algorithm must be ${Dilithium.NAME}" }
        require(publicKey.metadata.algorithm == Dilithium.NAME) { "Public key algorithm must be ${Dilithium.NAME}" }
    ...
    private fun JavaPrivateKey.asSigningPrivateKey(random: SecureRandom): SigningPrivateKey = JavaSigningPrivateKeyAdapter(this, random)
