...
fun Access.Companion.unauthenticated(origin: Origin = Origin.create(), authorization: AuthorizationPrincipal = AuthorizationPrincipal.withoutRoles()): Access.Unauthenticated<ULID, Authentication> = Access.Unauthenticated(origin, authorization)

context(WithCoreGenerators)
fun Access.Unauthenticated.Companion.create(origin: Origin = Origin.create(), authorization: AuthorizationPrincipal = AuthorizationPrincipal.withoutRoles()): Access.Unauthenticated<ULID, Authentication> = Access.Unauthenticated(origin, authorization)
