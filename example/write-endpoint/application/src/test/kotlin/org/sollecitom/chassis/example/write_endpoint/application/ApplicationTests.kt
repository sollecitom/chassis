...
private class ApplicationTests : CoreDataGenerator by CoreDataGenerator.testProvider {
    ...
    private fun TestScope.newApplication(events: EventFramework.Mutable = EventFramework.Mutable.inMemory(queryFactory = UserEvent.inMemoryQueryFactory), userRepository: UserRepository = EventSourcedUserRepository(events = events, coreDataGenerators = this@ApplicationTests)): Application = Application(userRepository::withEmailAddress)
}